...
export async function up(knex: Knex): Promise<void> {
  ...
  const rows: any[] = (await knex.raw('SHOW INDEX FROM text_discourse WHERE Key_name=?', indexName))[0];
...
export async function down(knex: Knex): Promise<void> {
  return knex.schema.table('text_discourse', (table) => {
    table.dropIndex('explicit_spelling', 'idx_text_discourse_explicit_spelling');
  });
}
