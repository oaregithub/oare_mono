...
export async function up(knex: Knex): Promise<void> {
  ...
  const columnRows: any[] = (await knex.raw(`SHOW COLUMNS FROM text LIKE '${columnName}'`))[0];
  ...
  if (columnRows.length === 0) {
    ...
    const textNames = await Promise.all(textUuids.map(({ uuid }) => aliasName(uuid)));

    await Promise.all(
      textUuids.map(({ uuid }, index) => knex('text').update(columnName, textNames[index]).where({ uuid })),
    );
  }
}
