...
export async function up(knex: Knex): Promise<void> {
  ...
  const hasOfficial1Column = await knex.schema.hasColumn('period', 'official1_uuid');
  const hasOfficial2Column = await knex.schema.hasColumn('period', 'official2_uuid');
  const hasOfficialPN1Column = await knex.schema.hasColumn('period', 'officialPN1_uuid');
  const hasOfficialPN2Column = await knex.schema.hasColumn('period', 'officialPN2_uuid');
  const hasPeriodTypeColumn = await knex.schema.hasColumn('period', 'period_type');
