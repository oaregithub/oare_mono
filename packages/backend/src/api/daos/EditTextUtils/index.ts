...
class EditTextUtils {
  ...
  async editSign(
    ...
    // DAMAGE
    const existingDamage = existingMarkup.filter(m => m.type === 'damage');
    const payloadDamage = payload.markup.filter(m => m.type === 'damage');
    ...
    if (existingDamage.length >= payloadDamage.length) {
      ...
      const uuidsToRemove = await k('text_markup')
        .pluck('uuid')
        .where({ reference_uuid: payload.uuid, type: 'damage' })
    ...
    const removedMarkup = existingMarkup
      ...
      .filter(
        m =>
          m !== 'damage' && !payload.markup.map(mark => mark.type).includes(m)
      );
